...
export default function RadialOrbitalTimeline({ timelineData }: RadialOrbitalTimelineProps) {
  ...
  zIndex: isExpanded ? 99999 : position.zIndex,
  opacity: isExpanded
  ? 1
  : dimmed
    ? 0.08
    : position.opacity,
              ...
              >
                ...
                <div
  className={`absolute top-12 z-0 whitespace-nowrap text-xs font-semibold tracking-wider transition-all duration-300 ${
    isExpanded
      ? "hidden"
      : "text-white/70"
  }`}
>
  {item.title}
                ...
                {isExpanded && (
                  <Card
  className="absolute top-20 left-1/2 -translate-x-1/2 w-64 rounded-lg 
                    ...
                    <CardHeader className="pb-2">
                      <div className="flex justify-between items-center">
                        <Badge className={`px-2 text-xs ${
                          item.status === "completed" ? "text-white bg-black border-white"
                          : item.status === "in-progress" ? "text-black bg-white border-black"
                          : "text-white bg-black border-white/50"
                        }`}>
                    ...
                    <CardContent className="text-xs text-white/80">
                      ...
                      <div className="mt-4 pt-3 border-t border-white/10">
                        <div className="flex justify-between items-center text-xs mb-1">
                          <span className="flex items-center"><Zap size={10} className="mr-1" />Effective Level</span>
                            ...
                            {item.relatedIds.map((relatedId) => {
                              ...
                              return (
                                <Button
                                  ...
                                  variant="outline"
                                  size="sm"
                                  className="flex items-center h-6 px-2 py-0 text-xs rounded-none border-white/20 bg-black hover:bg-white/10 text-white/80 hover:text-white transition-all"
